...
import { useAdminPushTemplates } from '../services/admin.queries';
...
export const PushTemplatesPage: React.FC = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => api.post(`/v1/admin/templates/push`, data),
    onSuccess: () => {
  ...
  const { data, isLoading } = useAdminPushTemplates({
  ...
  return (
    <div className="space-y-6 max-w-full overflow-hidden">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-text-primary tracking-tight">
            Push Templates <span className="text-text-secondary text-lg font-medium">({data?.meta?.total || 0})</span>
          </h1>
        </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-surface border border-border rounded-xl w-full max-w-md p-6">
            <h2 className="text-xl font-bold text-text-primary mb-4">Create Push Template</h2>
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                <label className="block text-sm text-text-secondary mb-1">Internal Name</label>
                <input name="name" required className="w-full bg-background border border-border rounded px-3 py-2 text-text-primary" />
              </div>
              <div>
                <label className="block text-sm text-text-secondary mb-1">Notification Title</label>
                <input name="title" required className="w-full bg-background border border-border rounded px-3 py-2 text-text-primary" />
              </div>
              <div>
                <label className="block text-sm text-text-secondary mb-1">Notification Body</label>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <KPICard title="Sent" value={data?.kpis?.sent || '0'} loading={isLoading} className="border-brand-primary/20" />
        <KPICard title="Global CTR" value={data?.kpis?.ctr || '0%'} loading={isLoading} className="border-warning/20" />
      </div>
...
export default PushTemplatesPage;
