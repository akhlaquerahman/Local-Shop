...
import { Plus, X, Shield, Users, UserCheck, Search, Download } from 'lucide-react';
...
import { useAdminAgents, useAdminAgentRoles, useCreateAdminAgent, useUpdateAdminAgent, useDeleteAdminAgent } from '../services/admin.agents.queries';
...
const AdminAgentsPage: React.FC = () => {
  ...
  const { data: roles = [], isLoading: isLoadingRoles } = useAdminAgentRoles();
  ...
  const onSubmit = async (formData: AgentFormValues) => {
    try {
      ...
      if (editingAgent) {
        await updateMutation.mutateAsync({ id: editingAgent._id, data: payload });
        addToast({ title: 'Success', message: 'Agent account updated', type: 'success' });
      } else {
        await createMutation.mutateAsync(payload);
        addToast({ title: 'Success', message: 'Agent account created', type: 'success' });
      }
  ...
  const handleDeactivate = async (id: string, currentStatus: string) => {
    ...
    try {
      ...
      addToast({ title: 'Success', message: `Agent ${newStatus.toLowerCase()}`, type: 'success' });
    } catch (err: any) {
      addToast({ title: 'Error', message: err.message, type: 'error' });
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Remove this agent? This will revoke all their access to the platform.')) {
      try {
        await deleteMutation.mutateAsync(id);
        addToast({ title: 'Success', message: 'Agent removed', type: 'success' });
      } catch (err: any) {
        addToast({ title: 'Error', message: err.message, type: 'error' });
      }
    }
  ...
  const columns = [
    ...
    { header: 'Employee ID', accessorKey: 'employeeCode', cell: (row: any) => <span className="font-mono text-xs font-bold text-text-primary">{row.employeeCode}</span> },
    ...
    { header: 'Role', accessorKey: 'role', cell: (row: any) => (
        <span className="font-semibold text-[10px] px-2 py-1 bg-surface border border-border rounded text-text-primary uppercase tracking-wider">
          {row.roleId?.name || row.role || 'AGENT'}
        </span>
      )
    },
    ...
    { header: 'Date Joined', accessorKey: 'dateOfJoining', cell: (row: any) => <span className="text-xs text-text-secondary">{row.dateOfJoining ? new Date(row.dateOfJoining).toLocaleDateString() : '-'}</span> },
    {
      ...
      cell: (row: any) => (
        <span className="text-xs bg-surface border border-border px-2 py-1 rounded">
          {row.effectivePermissions?.length || 0} Enabled
        </span>
      )
    },
    {
  ...
  return (
    <div className="space-y-6 text-left max-w-[1440px] mx-auto pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-text-primary tracking-tight">Agent Management</h1>
          <p className="text-sm text-text-secondary">Control access and roles for your platform employees</p>
        </div>
        <Button onClick={() => { setEditingAgent(null); methods.reset(defaultValues); setIsDrawerOpen(true); }} icon={<Plus size={16} />}>Create Agent</Button>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KPICard title="Total Agents" value={data?.stats?.total || 0} loading={isLoading} icon={<Users className="text-primary" />} />
        <KPICard title="Active Agents" value={data?.stats?.active || 0} loading={isLoading} icon={<UserCheck className="text-success" />} className="border-success/20" />
        ...
        <div className="bg-surface border border-border rounded-xl p-4 flex items-center justify-between cursor-pointer hover:bg-surface/80">
          <div>
      ...
      <div className="bg-surface border border-border rounded-xl flex flex-col overflow-hidden">
        <div className="p-4 border-b border-border flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="relative w-full sm:w-96">
            ...
            <input 
              type="text" 
              placeholder="Search agent by name, ID or email..." 
              value={search}
        ...
        <DataTable 
          ...
          isLoading={isLoading}
          exportFileName="platform-agents-list"
          emptyState={
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-text-secondary max-w-sm mb-6">
                Create platform agent accounts to securely grant access to specific parts of your super admin portal.
              </p>
              <Button onClick={() => { setEditingAgent(null); methods.reset(defaultValues); setIsDrawerOpen(true); }} icon={<Plus size={16} />}>
                Create First Agent
              </Button>
            </div>
          }
        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full max-w-2xl bg-surface h-full shadow-2xl animate-in slide-in-from-right overflow-y-auto flex flex-col">
            <div className="p-6 border-b border-border flex justify-between items-center sticky top-0 bg-surface z-10">
              <div>
                <h2 className="text-xl font-bold text-text-primary">{editingAgent ? 'Edit Agent Account' : 'Create Agent Account'}</h2>
                <p className="text-xs text-text-secondary mt-1">{editingAgent ? 'Update platform employee details' : 'Fill in the details to add a new employee.'}</p>
              </div>
            ...
            <div className="flex-1 p-6">
              <FormProvider {...methods}>
                <form id="create-agent-form" onSubmit={methods.handleSubmit(onSubmit)} className="space-y-8">
                  
                  {/* Read Only Meta */}
                  <div className="bg-background border border-border p-4 rounded-xl flex items-center justify-between">
                    <div>
                      <p className="text-[10px] uppercase font-bold tracking-wider text-text-secondary">Portal Access</p>
                      <p className="font-semibold text-sm">Super Admin Dashboard</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] uppercase font-bold tracking-wider text-text-secondary">Employee Code</p>
                      <p className="font-mono font-bold text-sm text-primary">{editingAgent ? editingAgent.employeeCode : 'AUTO-GENERATED'}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <FormInput name="email" label="Email Address" type="email" placeholder="jane@platform.com" />
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FormInput name="password" label="Password" type="password" placeholder={editingAgent ? 'Leave blank to keep same' : ''} />
                      <FormInput name="confirmPassword" label="Confirm Password" type="password" placeholder={editingAgent ? 'Leave blank to keep same' : ''} />
                    </div>
                        ...
                        render={({ field }) => (
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {roles.map((r: any) => (
                              ...
                              >
                                <div className={`p-2 rounded-md ${field.value === r._id ? 'bg-primary text-white' : 'bg-surface text-text-secondary'}`}>
                                  <Shield size={16} />
                                </div>
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center justify-between mb-0.5">
                                    <h4 className={`text-sm font-bold truncate ${field.value === r._id ? 'text-primary' : 'text-text-primary'}`}>{r.name}</h4>
                                    {r.isSystem && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-primary/10 text-primary border border-primary/20">SYSTEM</span>}
                                  </div>
                            ...
                            {roles.length === 0 && (
                              <div className="col-span-2 text-sm text-text-secondary p-4 bg-background border border-border rounded-lg text-center">
                                No roles defined yet. Please create a role via API/DB first.
                              </div>
                            )}
                          </div>
                        )}
            ...
            <div className="p-6 border-t border-border bg-surface sticky bottom-0 flex justify-end gap-3 z-10">
              <div className="flex gap-4 w-full">
                ...
                <Button type="submit" form="create-agent-form" className="flex-1" isLoading={createMutation.isPending || updateMutation.isPending}>
