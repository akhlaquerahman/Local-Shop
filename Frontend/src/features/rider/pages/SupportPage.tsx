...
import { Plus, X, MessageSquare, Clock, CheckCircle, Search } from 'lucide-react';
...
import { useRiderSupportTickets, useCreateSupportTicket } from '../services/rider.queries';
...
const ticketSchema = z.object({
  ...
  category: z.enum(['Delivery Issue', 'Payouts', 'Account', 'Technical', 'Other']),
...
export const SupportPage: React.FC = () => {
  ...
  const { data: tickets, isLoading, refetch } = useRiderSupportTickets();
  ...
  const columns = [
    { id: 'ticketNumber', header: 'Ticket ID', accessorKey: 'ticketNumber', cell: (row: any) => <span className="font-mono text-xs font-bold">{row.ticketNumber || row._id.substring(0,8)}</span> },
    ...
    { id: 'status', header: 'Status', accessorKey: 'status', cell: (row: any) => (
        <span className={`px-2 py-1 text-[10px] font-bold rounded-full uppercase tracking-wider ${row.status?.toLowerCase() === 'resolved' || row.status?.toLowerCase() === 'closed' ? 'bg-success/10 text-success' : 'bg-primary/10 text-primary'}`}>
          {row.status}
        </span>
      )
    },
  ...
  const filteredData = tickets
    ?.filter((t: any) => t.subject?.toLowerCase().includes(search.toLowerCase()) || t.ticketNumber?.toLowerCase().includes(search.toLowerCase()) || t._id.toLowerCase().includes(search.toLowerCase()))
    ?.map((t: any) => ({ ...t, id: t._id })) || [];
  
  // Calculate stats
  const openCount = tickets?.filter((t:any) => t.status?.toLowerCase() === 'open').length || 0;
  const inProgressCount = tickets?.filter((t:any) => t.status?.toLowerCase() === 'in_progress' || t.status?.toLowerCase() === 'pending').length || 0;
  const resolvedCount = tickets?.filter((t:any) => t.status?.toLowerCase() === 'resolved' || t.status?.toLowerCase() === 'closed').length || 0;

  return (
    <div className="space-y-6 text-left max-w-[1440px] mx-auto pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-text-primary tracking-tight">Rider Help Desk</h1>
          <p className="text-sm text-text-secondary">Get support for deliveries, payouts, and account issues</p>
        </div>
        {!showForm && (
      ...
      {showForm ? (
            ...
            <form onSubmit={methods.handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormSelect name="category" label="Category" options={[
                  ...
                  { label: 'Payouts & Earnings', value: 'Payouts' },
        ...
        <div className="bg-surface border border-border rounded-xl flex flex-col overflow-hidden">
          ...
          <DataTable 
            ...
            data={filteredData} 
            isLoading={isLoading}
            exportFileName="rider-support-tickets"
          />
        </div>
      )}
