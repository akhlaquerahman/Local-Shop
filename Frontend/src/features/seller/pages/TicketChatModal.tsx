...
import { X, Send, User, Clock, CheckCircle, Shield, CheckCircle2 } from 'lucide-react';
import { Button } from '@/components/ui/Button';
import { useTicketById, useReplyTicket, useResolveTicket } from '../services/seller.queries';
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6 bg-background/50">
          {isLoading ? (
            ...
            ticket?.messages?.map((msg: any, idx: number) => {
              ...
              return (
                <div key={idx} className={`flex gap-3 ${isSeller ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${isSeller ? 'bg-primary/20 text-primary' : isAdmin ? 'bg-danger/20 text-danger' : 'bg-surface border border-border text-text-secondary'}`}>
                    {isSeller ? <User size={14} /> : isAdmin ? <Shield size={14} /> : <User size={14} />}
                  </div>
                  <div className={`max-w-[80%] rounded-2xl p-4 ${isSeller ? 'bg-primary text-white rounded-tr-none' : isAdmin ? 'bg-surface border border-danger/20 rounded-tl-none' : 'bg-surface border border-border rounded-tl-none'}`}>
                    ...
                    <p className={`text-sm whitespace-pre-wrap ${isSeller ? 'text-white' : 'text-text-primary'}`}>{msg.message}</p>
                  </div>
                </div>
              );
            })
